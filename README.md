...
| [InternAgent-1.5](https://github.com/InternScience/InternAgent) | Unified agentic framework for **long-horizon** autonomous scientific discovery (Shanghai AI Lab / InternScience, arXiv 2602.08990) | Three-subsystem loop — Generation (literature + hypothesis) → Verification (computation, simulation or wet lab) → Evolution (refine & propagate); persistent cross-session memory so failed directions aren't re-run; leads GAIA / HLE / GPQA / FrontierScience; covers both algorithm discovery and empirical discovery across physics, biology, earth & life science |
| [MLEvolve](https://github.com/InternScience/MLEvolve) | Self-evolving multi-agent system for end-to-end **ML algorithm discovery and optimization** (InternScience, arXiv 2606.06473) | Progressive MCGS extends tree search with cross-branch reference edges and an entropy-inspired explore→exploit schedule; Retrospective Memory pairs a cold-start domain knowledge base with dynamic global memory so experience is retrieved and reused across tasks; adaptive coding modes decouple strategic planning from code generation for stable long-horizon iteration; SOTA medal rate and valid-submission rate on MLE-Bench under a **12-hour budget (half the standard runtime)**, and beats AlphaEvolve on math algorithm optimization |
...
| [Agent Lightning](https://github.com/microsoft/agent-lightning) | Microsoft's RL trainer that post-trains agents **inside the harness you actually ship** (v1.0, Aug 2026, arXiv 2608.17528) | MIT, 17.5K+ stars; ~3,500 lines of core Python; zero agent code changes — the agent reaches the model through an Agent Lightning proxy while tools, context, control flow and environments stay in the loop; verl + vLLM backend; native Kubernetes Jobs instead of an external sandbox service; 6K training samples took Qwen3.5-9B from 41.8% → 56.4% on SWE-bench Verified |
| [AgentGym-RL](https://github.com/WooooDyy/AgentGym-RL) | Modular framework for training LLM agents on **long-horizon multi-turn decision making** from scratch (ICLR 2026) | Fully decoupled environment / agent / training layers; PPO, GRPO, RLOO, REINFORCE++ plus complementary SFT / DPO / AgentEvol paradigms; five real-world scenario families (web navigation, deep search, digital games, embodied control, science tasks); **ScalingInter-RL** staged curriculum grows the interaction horizon from short to long for stable long-horizon training; trained agents match or surpass OpenAI o3 and Gemini-2.5-Pro across 27 tasks |
...
| [DataArc-SynData-Toolkit](https://github.com/DataArcTech/DataArc-SynData-Toolkit) | Unified **closed-loop** framework for multi-path, multimodal & multilingual data synthesis (DataArcTech + IDEA, arXiv 2605.08138) | 1.8K+ stars; config-driven end-to-end pipeline with both a visual UI and a simplified CLI; local synthesis / HuggingFace dataset retrieval / model distillation as interchangeable synthesis paths; quality-controllable loop runs through to downstream fine-tuning; strong low-resource-language coverage |
...
| [KDFlow](https://github.com/songmzhang/KDFlow) | Decoupled LLM distillation framework — **SGLang for the teacher, FSDP2 for the student** (arXiv 2603.01875) | Fixes the systems mismatch of running a forward-only teacher inside the training backend; zero-copy hidden-state transfer + student-side logit recompute gives efficient full-vocabulary KD; off-policy / on-policy (OPD) / cross-tokenizer / multimodal / on-policy self-distillation, with multi-teacher, chunked loss and EMA teacher updates in v0.2.0; built-in Vanilla KD + DSKD and easy custom-algorithm registration; **1.4–6x faster** than mainstream KD frameworks |
...
| [VLA Foundry](https://github.com/TRI-ML/vla_foundry) | Toyota Research Institute's unified framework for training **LLMs, VLMs and VLAs in a single codebase** (arXiv 2604.19728) | Train an LLM → reuse the checkpoint to train a VLM → reuse again for a VLA, without switching codebases or adding external deps; most modules pure PyTorch; FSDP2 + WebDataset streaming; torchrun locally, AWS SageMaker/S3 on large clusters; pretrained LLM/VLM/VLA checkpoints released on the Hub |
...
| [RL-Insight](https://github.com/verl-project/rl-insight) | Online observability purpose-built for **RL training loops** (verl project) | One-command Prometheus + Tempo + Grafana stack; unifies actor, rollout, transfer-queue, engine and hardware metrics with RL state timelines in a single view while keeping training-side instrumentation lightweight; framework-agnostic data protocol; upstreamed into verl (v0.2.0, Jul 2026) and verl-omni |
...
| [AARR / AARRI-Bench](https://arxiv.org/abs/2606.07462) | Benchmark suite scoring frontier LLMs **and the agentic harnesses around them** across the research lifecycle | Tests professionalism, thoroughness and nuanced scientific judgment in granular research scenarios instead of macro-level task completion; best configuration (mini-swe-agent + Claude Opus 4.7) reaches only 68.3%; finds minimalist scaffolds providing low-level primitives beat feature-heavy designs; probes field sensitivity and research ethics |
| [SWE-bench Multimodal v2](https://www.swebench.com/multimodal.html) | Visual software-engineering benchmark, **fully open-sourced 2026-09-01** | 480 reproducible tasks retained from the original 517, with known flaky and ungradeable tests removed; issues carry screenshots of bugs, design mockups, diagrams and visual error context, so agents must act on image + text together; Docker environments rebuilt to fix dependency and browser drift; complete test split + evaluation tooling now runnable locally |
...
| [Prime Agent](https://github.com/PrimeIntellect-ai/prime-agent) | Prime Intellect's self-improving **RLM harness** — sub-agents are function calls inside a persistent IPython kernel (Aug 2026, arXiv 2608.23552) | MIT; the Recursive Language Model treats context as a variable and `rlm(...)` spawns a real child agent whose result returns into the session as data; **Continual Harness** keeps supplemental prompts, memories, skill descriptions and reusable subagent specs as durable state the agent refines through small evidence-backed updates; daemon-backed sessions, IPython state and schedules survive terminal detach; agent-to-agent messaging between live and retained subagents; built for long-running research evaluations; 95.5% on ARC-AGI-3 with Opus 5 |
...
24. **Training Moves Inside the Harness You Ship**: Sep 2026 — the newest frameworks stop asking you to rebuild your system for training. Microsoft's **Agent Lightning v1.0** post-trains an agent *in its deployment harness*: tools, context, control flow and environments stay in the loop while the agent reaches the model through a drop-in proxy with zero code changes (~3,500 lines of core Python on verl + vLLM, Kubernetes Jobs as the sandbox; 6K samples → +14.6 pts on SWE-bench Verified). The same train/serve collapse shows up across the stack — **VLA Foundry** keeps LLM → VLM → VLA pretraining in one codebase instead of three handoffs, **InternAgent-1.5** gives the discovery loop persistent cross-session memory so failed directions aren't re-run, and **DataArc-SynData-Toolkit** closes synthesis → fine-tuning into a single quality-controlled loop. Evaluation is following suit: **AARRI-Bench** scores the *harness*, not just the model, and still finds the best configuration at only 68.3%
25. **Research Loops Grow a Memory; the RL Stack Grows an Ops Tier**: Sep 2026 — two complementary maturations. On the research side, the fix for "every experiment starts from scratch" is now concrete machinery: **MLEvolve** replaces one-shot tree search with Progressive MCGS (cross-branch reference edges, entropy-scheduled explore→exploit) plus a Retrospective Memory that retrieves prior task experience — reaching SOTA on MLE-Bench at *half* the standard 24h budget and beating AlphaEvolve at math algorithm optimization. **Prime Agent** pushes the same idea into the harness itself: its Continual Harness treats prompts, skills and reusable subagent specs as durable state the agent edits from evidence, while sub-agents become literal function calls in a persistent REPL. On the systems side, RL is acquiring the operational tier ordinary training has had for years — **RL-Insight** gives rollout / actor / transfer-queue telemetry a Grafana-grade home (now upstream in verl), **KDFlow** splits the distillation pipeline along its real seam (SGLang for the forward-only teacher, FSDP2 for the student) for 1.4–6x speedups, and **AgentGym-RL** turns long-horizon stability into a configurable curriculum (ScalingInter-RL) instead of a tuning art. Evaluation keeps pace: **SWE-bench Multimodal v2** opened its full 480-task split for local reproducible runs
...
*Compiled March 2026, updated 2026-09-18. Project statuses may change — check individual GitHub repos for the latest.*
