...
- **关键特性**:
  - **双层自进化**：过去的试验改变未来的研究行为；反复出现的**流程性失败**则改变 Harness 本身
...
### 1.20 InternAgent-1.5（上海 AI Lab）⭐⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/InternScience/InternAgent
- **论文**: https://arxiv.org/abs/2602.08990（InternAgent-1.5: A Unified Agentic Framework for Long-Horizon Autonomous Scientific Discovery）
- **来源**: InternScience —— 上海人工智能实验室 AI for Science 中心开源阵地
- **核心理念**: 统一的**长周期**自主科学发现框架——把「提出假设 → 验证假设 → 从结果中进化」组织成一个可持续迭代的闭环，而不是一次性的研究流水线
- **关键特性**:
  - **三大子系统持续循环**：**生成子系统**做大规模文献分析与科学推理，产出结构化假设；**验证子系统**通过计算分析、仿真或**物理湿实验**评估假设；**进化子系统**解读结果、识别局限，生成精炼认知指导下一轮发现
  - **持久化记忆模块**：跨会话记录实验结果，让 Agent 避开此前已失败的方向、在成功方向上继续累积，而非每轮重启
  - **双类任务覆盖**：*算法发现*（为核心机器学习问题自主设计有竞争力的方法）+ *经验发现*（在地球、生命、生物、物理领域执行完整的计算或湿实验并产出科学结论）
  - 在 GAIA、HLE、GPQA、FrontierScience 等科学推理基准上取得领先表现；1.4K+ stars，1.0 版本自 2025-10 起已全量开源
- **适用场景**: 需要横跨多学科的长周期自主科研；既要算法层面自动设计、又要接入真实实验设备的科学发现闭环

### 1.21 MLEvolve（上海 AI Lab / InternScience）⭐⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/InternScience/MLEvolve
- **论文**: https://arxiv.org/abs/2606.06473（MLEvolve: A Self-Evolving Framework for Automated Machine Learning Algorithm Discovery）
- **来源**: InternScience（与 InternAgent 同源）
- **核心理念**: 基于 LLM 的自进化多 Agent 框架，做**端到端机器学习算法发现与优化**——用「渐进式搜索 + 经验驱动记忆」把算法设计变成可长程迭代的自主过程
- **关键特性**:
  - **Progressive MCGS（渐进式蒙特卡洛图搜索）**：把树搜索扩展为图搜索，通过基于图的引用边实现**跨分支信息流动**；配合受熵启发的渐进式调度，把搜索从广泛探索逐步收敛到集中利用
  - **Retrospective Memory（回溯记忆）**：冷启动领域知识库 + 动态全局记忆，支持按任务检索并复用既往经验，让 Agent 随经验累积而进化，而非每次从零开始
  - **自适应编码模式**：把战略规划与代码生成解耦，保证长程迭代的稳定性
  - **实测**：在 MLE-Bench 上以 **12 小时预算（标准运行时长的一半）** 取得平均夺牌率与有效提交率双 SOTA；在数学算法优化任务上超越 AlphaEvolve 等专用算法发现方法，展现跨领域泛化能力
- **适用场景**: 自动化 ML 算法设计与调优；需要在有限算力预算内跑 Kaggle 式端到端 ML 工程任务的自主 Agent
...
- **核心理念**: 首个面向**扩散语言模型（Diffusion LLM）**的强化学习后训练框架——把「偏好推理轨迹」引入 diffusion LLM 的后训练
- **关键特性**:
  - **TraceRL（轨迹感知 RL）**：直接优化偏好推理轨迹，跨不同扩散架构通用
  - 引入**基于扩散的价值模型**增强训练稳定性、降低方差；可选过程奖励模型做细粒度监督
...
- **核心理念**: 面向**企业生产级**的大规模 RL 后训练框架——从 slime fork 并与之协同演进，专注大规模 MoE 训练与生产落地
- **关键特性**:
...
- **关键特性**:
  ...
  - **AutoTool（ICML 2026）**：让 Agent 在推理轨迹中具备**动态工具选择**能力，而非固定工具集
...
- **核心理念**: 端到端 RL 训练工具调用型 LLM Agent——把每一轮交互建模为 step-level MDP 转移
- **关键特性**:
  - 将工具使用、环境状态、上下文管理、奖励分配、策略优化统一为同一个可训练底座
  - **StepPO（2026-05）**：步骤级策略优化算法，配套扩展 recipe 覆盖
  - 内置 HotpotQA、ALFWorld、WebShop、学术论文检索等多种任务环境
- **适用场景**: 多轮工具调用 Agent 的强化学习训练；step-level 奖励建模

### 4.27 Agent Lightning（微软）⭐⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/microsoft/agent-lightning
- **论文**: https://arxiv.org/abs/2608.17528（Agent Lightning v1.0: Towards Harnessed Agentic RL）
- **发布**: v1.0 — 2026-08-17（完全重写版），MIT 协议，17.5K+ stars
- **来源**: Microsoft Research
- **核心理念**: **Harnessed Agentic RL（带 Harness 的智能体强化学习）**——直接在「你上线时真正使用的那套 Harness」里做后训练，把训练环境与部署环境的差距抹平，而不是为了训练把 Agent 重写一遍
- **关键特性**:
  - **Agent 代码零改动**：Agent 通过 Agent Lightning 代理（proxy）访问模型，工具、上下文、控制流、环境全部保留在训练回路中——现有生产 Agent 可直接接入
  - **约 3,500 行核心 Python**：v1.0 是彻底重新设计与重实现的版本，以简洁性为第一优先级，便于审计与二次开发
  - **基础设施组合**：训练侧接 **verl**，推理侧接 **vLLM**
  - **原生 Kubernetes 支持**：Agent 直接作为 Kubernetes Job 运行，无需依赖外部沙箱服务
  - **完整编码 Agent 训练示例**：涵盖数据清洗、奖励黑客（reward hacking）防范与训练脚本
  - **实测**：仅用 6K 条训练样本与适度算力，Qwen3.5-9B 在 SWE-bench Verified 上从 41.8% 提升到 56.4%（绝对 +14.6 个百分点）
- **适用场景**: 已有生产级 Agent、希望直接用真实工作流做 RL 后训练；在 Kubernetes 集群上跑 Agentic RL

### 4.28 AgentGym-RL ⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/WooooDyy/AgentGym-RL
- **论文**: AgentGym-RL: Training LLM Agents for Long-Horizon Decision Making through Multi-Turn Reinforcement Learning（ICLR 2026）
- **项目主页**: https://agentgym-rl.github.io/
- **核心理念**: 模块化、完全解耦的**多轮 RL 框架**——不依赖 SFT 冷启动，从零训练 LLM Agent 做长程决策
- **关键特性**:
  - **三层解耦架构**：环境、Agent、训练三部分互不耦合，灵活可扩展，便于接入自定义环境与算法
  - **算法覆盖广**：主流在线 RL（PPO、GRPO、RLOO、REINFORCE++）之外，还支持 SFT、DPO、AgentEvol 等互补训练范式
  - **五类真实场景**：网页导航、深度搜索、电子游戏、具身控制、科学任务，合计 27 个任务
  - **ScalingInter-RL（交互轮次渐进扩展）**：分阶段训练策略——先用短交互建立基础策略，再逐步放大交互轮次鼓励深度探索，显著缓解长程 RL 的训练不稳定问题
  - **实测**：训练后的开源小模型 Agent 在多个环境上达到甚至超越 OpenAI o3、Gemini-2.5-Pro 等商用模型
- **适用场景**: 长程多轮决策 Agent 的 RL 训练；需要在异构真实环境（Web / 游戏 / 具身 / 科学）上统一训练与评测的场景
...
- **来源**: 阿里 ModelScope（魔搭）
- **核心理念**: 高效的自进化 Agent 训练框架——让 Agent 自主提升能力，实现低成本、可持续的能力进化
- **关键特性**:
  - 统一 **self-questioning（自我提问）+ self-navigating（自我导航）+ self-attributing（自我归因）** 三大机制
  - **SeeUPO（2026-03）**：序列级 Agentic RL 框架，带收敛性保证，配套多轮训练栈
  - 减少对人工标注任务和外部奖励的依赖，自主生成训练信号
- **适用场景**: Agent 能力的持续自进化训练；低成本多轮 Agentic RL
...
- **GitHub**: https://github.com/DataArcTech/DataArc-SynData-Toolkit
- **论文**: https://arxiv.org/abs/2605.08138（DataArc-SynData-Toolkit: A Unified Closed-Loop Framework for Multi-Path, Multimodal, and Multilingual Data Synthesis）
- **来源**: DataArcTech + 粤港澳大湾区数字经济研究院（IDEA）
- **核心理念**: 统一的**闭环**数据合成框架——多路径、多模态、多语种合成与下游微调串成一条可质量管控的回路，降低现有合成数据工具的使用门槛
- **关键特性**:
  - **配置驱动的端到端流水线**：同时提供直观可视化界面与简化 CLI，配置文件即可跑通全流程
  - **多路径合成**：本地合成、HuggingFace 数据集检索、模型蒸馏三条路径可互换组合
  - **高度模块化架构**：为多模态、多语种、多任务适配而设计；对英语之外的**低资源语言**覆盖较好
  - **唯一覆盖「多策略 + 多模态合成 + 下游模型微调」完整链路**的开源框架（论文对比结论）；1.8K+ stars
- **适用场景**: 需要为特定领域或低资源语言批量造训练数据；希望合成与微调效果形成闭环验证而非一次性产出
...
- **发布**: 2026-07-13
- **核心理念**: 首个统一的**在线策略蒸馏（On-Policy Distillation, OPD）**框架——学生模型自己生成、教师模型在线打分，把 OPD 做得像微调一样可配置
- **关键特性**:
  - 覆盖 **10+ 种 OPD 方法**，通过**一行 YAML 切换**：跨 tokenizer OPD、在线自蒸馏（self-distillation）、步骤级（step-wise）OPD 三大设定
...
- **论文**: https://arxiv.org/abs/2603.01875（KDFlow: A User-Friendly and Efficient Knowledge Distillation Framework for Large Language Models）
- **核心理念**: **训推解耦**的 LLM 蒸馏框架——教师用 SGLang 推理、学生用 FSDP2 训练，让蒸馏流水线按其真实的系统边界拆开
- **关键特性**:
  - **修正既有框架的系统错配**：主流 KD 框架把教师和学生一起塞进同一个训练后端，但教师只跑前向、学生要跑前向+反向+优化器更新——KDFlow 将两侧分离，各用最合适的引擎
  - **高效全词表蒸馏**：教师隐状态以**零拷贝**方式传输，在学生侧重算 logits，避免搬运巨大的全词表 logits 张量
  - **方法覆盖全**：off-policy、on-policy（OPD）、跨 tokenizer（如 Llama → Qwen）、多模态、在线自蒸馏（OPSD）
  - **v0.2.0（2026-07）**：分块 loss、多教师蒸馏、OPSD 的 EMA 教师更新、动态 batch size；v0.1.3（2026-04）支持自蒸馏中学生→教师权重同步
  - 内置 Vanilla KD 与 DSKD（双空间知识蒸馏），可通过简洁 API 注册自定义算法
  - **相比主流 KD 框架提速 1.4x–6x**
- **适用场景**: 需要高吞吐大规模蒸馏的生产流水线；跨模型家族（不同 tokenizer）的知识迁移；与 EasyOPD 同属 OPD 工具链但更偏系统效率
...
- **核心理念**: 面向**多模态生成模型**的通用 RL 训练框架——"Easy, Fast, and Stable RL Training for Diffusion and Omni-Modality Models"
- **关键特性**:
...
- **发布**: v0.1.0 — 2026-08
- **来源**: verl 项目官方
- **核心理念**: 统一的**视觉-语言-动作（VLA）**策略后训练框架——把人在回路数据采集、监督微调、强化学习、策略评测收进同一套工作流
- **关键特性**:
...
### 12.9 VLA Foundry（丰田研究院 TRI）⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/TRI-ML/vla_foundry
- **论文**: https://arxiv.org/abs/2604.19728（VLA Foundry: A Unified Framework for Training Vision-Language-Action Models）
- **项目主页**: https://tri-ml.github.io/vla_foundry/
- **来源**: Toyota Research Institute（TRI-ML）
- **核心理念**: 在**同一套代码库**里训练 LLM、VLM 与 VLA——从语言预训练一路贯通到动作专家微调，中途无需切换框架
- **关键特性**:
  - **链式复用 checkpoint**：先训 LLM，用其 checkpoint 训 VLM，再用 VLM checkpoint 训 VLA，同一套训练循环、数据抽象与配置接口贯穿三个阶段
  - **纯 PyTorch 实现**：绝大多数模块不依赖外部库，便于阅读与改造
  - **分布式训练**：FSDP2 + WebDataset 流式读取，自动梯度累积、混合精度与 checkpoint 同步；本地 torchrun、集群走 AWS SageMaker / S3
  - **既可从零预训练，也可直接加载 HuggingFace 预训练骨干**；配套在 HF 上发布了 LLM / VLM / VLA 预训练权重集合
- **适用场景**: 具身智能全链路自研（语言 → 视觉语言 → 动作）；不希望在三套框架间搬运 checkpoint 的团队
...
### 13.5 RL-Insight（verl 官方）⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/verl-project/rl-insight
- **发布**: v0.2.0 — 2026-07-16（2026-06 首发 Online Monitor）
- **来源**: verl 项目官方
- **核心理念**: 专为 **RL 训练循环**设计的在线可观测性工具——把训练侧指标、RL 状态轨迹与服务端仪表盘串成一张图，补上 RL 栈长期缺失的「运维层」
- **关键特性**:
  - **一条命令起服务**：自动安装依赖并拉起 Prometheus + Tempo + Grafana 全套监控栈
  - **统一视图**：Actor、Rollout、TransferQueue、推理引擎、硬件指标与 RL 状态时间线汇聚在同一监控面板，而训练侧埋点保持轻量
  - **开箱即用的 Grafana 面板**：按训练指标、rollout 行为、引擎指标、RL 状态时间线分区组织
  - **框架无关的数据协议**：定义通用的性能洞察流水线，可泛化到不同训练框架
  - 已上游集成进 **verl** 与 **verl-omni**，覆盖训练、rollout、TransferQueue、trace 与硬件监控
- **适用场景**: 分布式 RL 训练的性能诊断与瓶颈定位；长时间 RL 后训练任务的在线运维
...
- **关键特性**:
  - 任务覆盖软件工程、系统管理、数据处理、安全、科学计算，以及**模型训练**——直接对应本报告关心的自动化训练场景
...
- **论文**: https://arxiv.org/abs/2606.07462（Act As a Real Researcher: A Suite of Benchmarks Evaluating Frontier LLMs and Agentic Harnesses in Research Lifecycle）
- **发布**: 2026-06
- **评估**: 前沿 LLM **及其外围 Agentic Harness** 在科研全生命周期中的表现；系列首个基准为 **AARRI-Bench**（Act As a Real Research Intern）
- **关键特性**:
  - 与只考察宏观执行能力的既有基准不同，AARR 关注 Agent 能否在**细粒度科研场景**中复现人类研究者的专业性、缜密度与微妙判断力
  - **同时评测模型与 Harness 的组合**——这正是自主研究工具选型时最缺的横向标尺
  - **实测**：最优配置（mini-swe-agent + Claude Opus 4.7）成功率仅 **68.3%**，常常忽略人类研究者一眼可见的关键细节
  - 结论呼应 mini-swe-agent 现象：复杂脚手架并非高分前提，提供底层原语的极简架构反而能胜过功能堆砌的设计
  - 明确指出前沿 Agent 在**领域敏感性、科研伦理、细致科学判断**上仍有显著短板，尚不能完全替代人类研究者
- **适用场景**: 为自主研究框架选型时横向对比「模型 × Harness」组合；评估自动化科研流程中哪些环节仍需人工把关

#### SWE-bench Multimodal v2 ⭐⭐⭐⭐ 🆕

- **官网**: https://www.swebench.com/multimodal.html
- **GitHub**: https://github.com/SWE-bench/SWE-bench
- **发布**: 2026-09-01 完全开源
- **评估**: Agent 在**同时含视觉信息的真实软件工程任务**上的表现——issue 中带有 bug 截图、设计稿/线框图、说明期望功能的示意图、含视觉上下文的报错
- **关键特性**:
  - 从原始 517 个 issue 中保留 **480 个可复现任务**，剔除已知不稳定（flaky）与无法评分的测试
  - **完整测试集与评测工具链全部开源**，可在本地运行，不再依赖托管评测
  - Docker 环境重建，修复依赖漂移与浏览器版本漂移问题
  - 与纯文本的 SWE-bench Verified 互补：检验编码 Agent 能否泛化到**视觉软件领域**（前端/UI 为主）
- **适用场景**: 评测多模态编码 Agent；检验 Agent 能否读懂截图/设计稿并据此改代码
...
- **关键特性**:
  ...
  - **一切皆插件**：模型适配器、工具注册表、会话日志、沙箱，乃至 **Agent 主循环本身**都是插件，由 Cordis 插件框架驱动
...
### 15.13 Live-SWE-agent ⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/OpenAutoCoder/live-swe-agent
- **核心理念**: 首个「运行时自进化」软件工程 Agent——在处理真实 issue 的过程中，**即时扩展和修订自身能力**
- **关键特性**:
  - 无需离线训练即可在任务中自我进化，动态生成/改写自己的工具与流程
  - Claude Opus 4.5 + Live-SWE-agent 在 **SWE-bench Verified 上达到 79.2%**
  - 与 self-evolving 训练范式呼应——把训练时改进与推理时改进的边界打通
- **适用场景**: 自主修复训练代码 Bug；研究运行时自进化 Agent 机制

### 15.14 mini-swe-agent ⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/SWE-agent/mini-swe-agent
- **来源**: SWE-agent 团队（Princeton + Stanford）
- **核心理念**: 仅 **100 行代码**的极简编码 Agent——无庞大配置、无 monorepo，可在命令行解决 GitHub issue
- **关键特性**:
  - **SWE-bench Verified 上 >74%**，以极简实现达到强基线
  - 被 Meta、NVIDIA、Essential AI、IBM、Nebius、Anyscale、普林斯顿、斯坦福等广泛采用
  - 易于嵌入自动化 pipeline，作为编写/调试训练脚本的轻量组件
- **适用场景**: 轻量级训练脚本编写与调试；可嵌入自动化训练循环的最小 Agent

### 15.15 Prime Agent（Prime Intellect）⭐⭐⭐⭐⭐ 🆕

- **GitHub**: https://github.com/PrimeIntellect-ai/prime-agent
- **论文**: https://huggingface.co/papers/2608.23552（Prime Agent: A Self-Improving RLM Harness）
- **官方博客**: https://www.primeintellect.ai/blog/prime-agent
- **发布**: 2026-08-05，MIT 许可
- **来源**: Prime Intellect
- **核心理念**: 自进化的 **RLM（Recursive Language Model，递归语言模型）Harness**——子 Agent 不是外部服务，而是常驻 IPython 内核里的**函数调用**
- **关键特性**:
  - **RLM 两大抽象**：上下文即变量（prompt-as-a-variable）；工具与递归子 Agent 即函数调用——在 REPL 里直接 `rlm(...)` 派生真实子 Agent，子 Agent 并行/后台执行，结果作为数据回到当前会话
  - **Continual Harness（持续进化 Harness）**：补充提示词、记忆、Skill 描述、可复用子 Agent 规格都作为**持久化状态**存储，Agent 通过小步、有证据支撑的更新不断精炼它们
  - **Agent 间直接通信**：运行中的 Agent 与已保留的子 Agent 可互相发现、交换消息、引导彼此正在进行的工作
  - **守护进程保障连续性**：会话、IPython 状态、定时任务与子 Agent 在终端断开后继续运行，可随时重新接入——专为长程任务（尤其是研究类评测）设计
  - **实测**：搭配 Opus 5 在 ARC-AGI-3 上取得 **95.5%**，略高于 95.4% 的人类专家基线
- **适用场景**: 长时间无人值守的训练/评测任务编排；需要 Harness 自身随经验演进的自主研究场景；与 Deli_AutoResearch、Sibyl 等「Harness 可进化」思路互为参照
...
MLEvolve（渐进式图搜索 + 回溯记忆，MLE-Bench 半预算 SOTA）
```
...
长程多轮决策 Agent RL: AgentGym-RL（ScalingInter-RL 渐进扩展交互轮次）
RL 训练可观测性: RL-Insight（Prometheus + Tempo + Grafana 一键起）
```
...
高吞吐蒸馏流水线: KDFlow（SGLang 教师 + FSDP2 学生，提速 1.4–6x）
在线策略蒸馏: EasyOPD（10+ 种 OPD 方法，一行 YAML 切换）
模型合并: MergeKit
...
19. **RL 拓展到新载体 + 全学科 Autoresearch**: RL 后训练正突破自回归文本模型——Gen-Verse 的 dLLM-RL/TraceRL 把轨迹感知 RL 带到**扩散语言模型**（SOTA TraDo 系列），vLLM 官方的 vime 把 RL 后训练标准化进推理引擎生态；与此同时，autoresearch 循环从 ML 泛化为全学科科研工作台（OpenScience：文献 → 假设 → 实验 → 论文，覆盖 ML/生物/物理/化学）
...
22. **Agent Harness 本身开源化、可插拔化**: DeepSeek 开源自家 Harness（两天 10 万 star），模型适配器、工具注册表、沙箱乃至 **Agent 主循环本身**都是插件；继 Claw Code 的 Clean-Room 重写之后，Harness 层正从厂商锁定变为通用开源基础设施；而 Terminal-Bench 2.1（斯坦福 + Laude，89 个任务含模型训练）正成为跨 Harness 的共同标尺
23. **自进化下沉到运行时**: 训练时改进与推理时改进的边界正在消失——**Live-SWE-agent** 无需离线训练，在处理真实 issue 的过程中即时改写自己的工具与流程（SWE-bench Verified 79.2%）；**AgentEvolver** 则在训练中自进化，用自我提问/自我导航/自我归因三段循环自主生成训练信号，摆脱人工标注任务与外部奖励。训练侧与之呼应的是 **Agent-R1**：把每一次工具调用建模为 step-level MDP 转移，让工具使用、记忆、奖励分配统一进同一个可训练底座——RL 训练对象直接变成多轮工具调用策略，而非单轮回复策略。**mini-swe-agent** 提供了反向参照：100 行代码即可在 SWE-bench Verified 上 >74%，说明能力并不来自脚手架的复杂度
24. **训练搬进「你真正上线的那套 Harness」**: 2026-09 新趋势——最新一批框架不再要求你为了训练把系统重写一遍。微软 **Agent Lightning v1.0**（2026-08-17，MIT）直接在部署用的 Harness 里做后训练：工具、上下文、控制流与环境全部留在训练回路中，Agent 通过一层代理访问模型、代码零改动（约 3,500 行核心 Python，训练接 verl、推理接 vLLM，Kubernetes Job 即沙箱；6K 样本让 Qwen3.5-9B 在 SWE-bench Verified 上 +14.6 个百分点）。同一种「训练/部署收敛」的思路贯穿整个栈：**VLA Foundry** 把 LLM → VLM → VLA 三段预训练收进一套代码库，不再在三个框架间搬 checkpoint；**InternAgent-1.5** 给科学发现循环加上跨会话持久记忆，让失败方向不再被重跑；**DataArc-SynData-Toolkit** 则把「合成 → 微调」闭成一个可质量管控的回路。评测侧同步跟进：**AARRI-Bench** 评的是「模型 × Harness」的组合而不只是模型，而最优配置至今仅 68.3%
25. **研究循环长出「记忆」，RL 技术栈长出「运维层」**: 2026-09 新趋势——两条并行的成熟化。研究侧，「每次实验都从零开始」的解法已经落到具体机制：上海 AI Lab / InternScience 的 **MLEvolve** 用 Progressive MCGS（图化树搜索 + 跨分支引用边 + 熵调度的探索→利用切换）替代一次性树搜索，再叠加 **Retrospective Memory** 按任务检索复用既往经验——在 MLE-Bench 上以**标准预算的一半（12 小时）** 拿下夺牌率与有效提交率双 SOTA，并在数学算法优化上超过 AlphaEvolve；**Prime Agent**（Prime Intellect，MIT）则把同一思路推进到 Harness 本身：Continual Harness 把提示词、Skill、可复用子 Agent 规格变成 Agent 可自行小步修订的持久状态，而子 Agent 干脆就是常驻 REPL 里的函数调用。系统侧，RL 正在补齐常规训练早已具备的运维层：**RL-Insight**（verl 官方，已上游集成）给 rollout / actor / TransferQueue 遥测配上 Grafana 级面板；**KDFlow** 沿蒸馏流水线的真实接缝切开（只跑前向的教师交给 SGLang，学生交给 FSDP2），换来 1.4–6x 提速；**AgentGym-RL**（ICLR 2026）把长程训练稳定性从「调参手艺」变成可配置课程（ScalingInter-RL 渐进扩展交互轮次）。评测同步跟进：**SWE-bench Multimodal v2**（2026-09-01）开放全部 480 个任务供本地可复现运行
...
*本报告基于 2026 年 9 月公开资源整理（2026-09-18 更新），项目状态可能随时间变化。建议定期检查各项目 GitHub 获取最新状态。*
